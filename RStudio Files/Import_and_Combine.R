...
# required by DADA2
seqtab_project_miseqrun1 <- as.matrix(
  read.delim(
...
# instead of ASV.
feattab_md5_project_miseqrun2 <- read.delim(
...
repseq_project_miseqrun2 <- getSequences(
...
feattab_repseq_project1_miseqrun2 <- cbind(
  repseq_project1_miseqrun2,
  feattab_md5_project1_miseqrun2
)
...
feattab_repseq_project1_miseqrun2$md5_agree <- ifelse(
  feattab_repseq_project1_miseqrun2$X_OTU_ID ==
    feattab_repseq_project1_miseqrun2$Feature,
  "Yes",
  ifelse(
    feattab_repseq_project1_miseqrun2$X_OTU_ID !=
      feattab_repseq_project1_miseqrun2$Feature,
...
feattab_md5_project1_miseqrun2_biom <- read_biom(
  "project1_miseqrun2_table-dada2.biom"
)
feattab_md5_project1_miseqrun2 <- as(
  biom_data(feattab_md5_project1_miseqrun2_biom),
  "matrix"
)
...
feattab_md5_project1_miseqrun2 <- as_tibble(
  feattab_md5_project1_miseqrun2,
  rownames = "Feature2"
)
...
# Feature (the md5 hash of the ASV, converted from the vector names in
# repseq.project1.miseqrun2)
repseq_project1_miseqrun2 <- getSequences(
...
# other than "X.OTU.ID", change this in the command below.
feattab_repseq_project1_miseqrun2$md5_agree <- ifelse(
  feattab_repseq_project1_miseqrun2$Feature2 ==
    feattab_repseq_project1_miseqrun2$Feature,
  "Yes",
  ifelse(
    feattab_repseq_project1_miseqrun2$Feature2 !=
...
# Convert the first column into rownames to match the format of the DADA2 output
row.names(seqtab_project1_miseqrun2) <- seqtab_project1_miseqrun2[, 1]

# Remove the 1st column, which is now a duplicate of the row names
seqtab_project1_miseqrun2 <- seqtab_project1_miseqrun2[, -1]

# Finally, convert the data.frame into a matrix, as required by DADA2
seqtab_project1_miseqrun2 <- as.matrix(seqtab_project1_miseqrun2)

# Look at your newly miseqrun2 sequence-table for project1.
View(seqtab_project1_miseqrun1)
...
seqlisttab_project1_miseqrun1 <- read.delim(
...
seqlisttab_project1_miseqrun1 <- subset(
  seqlisttab_project1_miseqrun1,
  select = -feature
)
# !!!!!OPTIONAL!!!!!
...
feattab_project1_miseqrun1_wide <- pivot_wider(
  seqlisttab_project1_miseqrun1,
  names_from = sample,
  values_from = count,
  values_fill = 0
...
# Transpose this feature-table into a sequence table, and convert the first
# column/row into headings.
seqtab_project1_miseqrun1 <- transpose(
  feattab_project1_miseqrun1_wide,
  keep.names = "Sample",
  make.names = 1
)

# Convert the first column into rownames to match the format of the DADA2 output
row.names(seqtab_project1_miseqrun1) <- seqtab_project1_miseqrun1[, 1]

# Remove the 1st column, which is now a duplicate of the row names
seqtab_project1_miseqrun1 <- seqtab_project1_miseqrun1[, -1]

# Finally, convert the data.frame into a matrix, as required by DADA2
seqtab_project1_miseqrun1 <- as.matrix(seqtab_project1_miseqrun1)
...
seqtab_project1 <- mergeSequenceTables(
  seqtab_project1_miseqrun1,
  seqtab_project1_miseqrun2,
  repeats = "sum",
  orderBy = "abundance"
)
